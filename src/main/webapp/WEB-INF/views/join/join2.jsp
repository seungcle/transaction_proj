	...
	<div class="login-card p-4">
		...
		<form action="${pageContext.request.contextPath}/join" method="post" id="registerForm">
			...
			<div class="d-grid gap-2 mt-4">
				<button type="submit" class="btn btn-success" id="submitBtn" disabled>가입하기</button>
			</div>
		</form>
	</div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('checkUsername').addEventListener('click', function() {
        ...
        if(username.length < 4) {
            document.getElementById('usernameFeedback').innerHTML='<span class="text-danger">아이디는 4자 이상이어야 합니다.</span>';
        ...
        xhr.onload = function(){
            ...
            if(res==='exists'){
                document.getElementById('usernameFeedback').innerHTML='<span class="text-danger">이미 사용 중인 아이디입니다.</span>';
                isUsernameChecked = false;
            } else {
                document.getElementById('usernameFeedback').innerHTML='<span class="text-success">사용 가능한 아이디입니다.</span>';
    ...
    document.getElementById('checkNickname').addEventListener('click', function() {
        ...
        if(nickname.length < 2) {
            document.getElementById('nicknameFeedback').innerHTML='<span class="text-danger">닉네임은 2자 이상이어야 합니다.</span>';
        ...
        xhr.onload = function(){
            const res = xhr.responseText.trim();
            if(res==='exists'){
                document.getElementById('nicknameFeedback').innerHTML='<span class="text-danger">이미 사용 중인 닉네임입니다.</span>';
                isNicknameChecked = false;
            } else {
                document.getElementById('nicknameFeedback').innerHTML='<span class="text-success">사용 가능한 닉네임입니다.</span>';
    ...
    document.getElementById('sendEmailBtn').addEventListener('click', function(){
        ...
        if(!email){
            document.getElementById('emailFeedback').innerHTML='<span class="text-danger">이메일을 입력하세요.</span>';
            return;
        }
        const xhr = new XMLHttpRequest();
        ...
        xhr.onload = function(){
            const res = xhr.responseText.trim();
            if(res==='success'){
                document.getElementById('emailFeedback').innerHTML='<span class="text-success">인증번호가 전송되었습니다.</span>';
                authCodeRow.style.display='table-row';
            } else {
                document.getElementById('emailFeedback').innerHTML='<span class="text-danger">인증번호 전송 실패</span>';
            }
        };
    ...
    document.getElementById('verifyEmailBtn').addEventListener('click', function(){
        ...
        if(!code){
            document.getElementById('authCodeFeedback').innerHTML='<span class="text-danger">인증번호를 입력하세요.</span>';
            return;
        }
        const xhr = new XMLHttpRequest();
        ...
        xhr.onload = function(){
            const res = xhr.responseText.trim();
            if(res==='success'){
                document.getElementById('authCodeFeedback').innerHTML='<span class="text-success">인증 성공!</span>';
                isEmailVerified = true;
                emailInput.readOnly = true;
                document.getElementById('sendEmailBtn').disabled = true;
                authCodeRow.style.display='none';
                hiddenEmail.value = emailInput.value.trim(); // 서버 전송용
                updateSubmitButtonState();
            } else {
                document.getElementById('authCodeFeedback').innerHTML='<span class="text-danger">인증번호가 일치하지 않습니다.</span>';
    ...
    nicknameInput.addEventListener('input', function(){
        isNicknameChecked=false;
        document.getElementById('nicknameFeedback').innerHTML='';
        updateSubmitButtonState();
    });
    emailInput.addEventListener('input', function(){
        isEmailVerified=false;
        document.getElementById('emailFeedback').innerHTML='';
        document.getElementById('authCodeFeedback').innerHTML='';
        authCodeRow.style.display='none';

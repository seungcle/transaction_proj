                ...
                <div id="address-list-container">
                    <% for (Map<String, Object> addr : addressList) { %>
                        ...
                        <div class="address-item position-relative <%= isDefault ? "is-default" : "" %>" data-address-id="<%= addr.get("id") %>">
                            ...
                            <p class="mb-0">[<%= addr.get("postcode") %>] <%= addr.get("address") %>, <%= addr.get("addressDetail") %></p>
                        </div>
                    <% } %>
                </div>
                
                <button class="btn btn-outline-secondary w-100" onclick="showView('form')">+ 배송지 추가</button>
            </div>

            <div id="address-form-view" class="d-none">
                <form onsubmit="return false;">
                    <div class="mb-3"><input type="text" class="form-control" placeholder="배송지명 (최대 10글자)"></div>
                    <div class="mb-3"><input type="text" class="form-control" placeholder="받는 분" value="이혜성"></div>
                    <div class="mb-3"><input type="tel" class="form-control" placeholder="연락처" value="01063454720"></div>
                    <div class="d-flex mb-2">
                        <input type="text" class="form-control" id="postcode" placeholder="우편번호" readonly>
                        <button type="button" class="btn btn-secondary ms-2 flex-shrink-0" onclick="showView('search')">주소 검색</button>
                    </div>
                    <div class="mb-3"><input type="text" class="form-control" id="address" placeholder="주소" readonly></div>
                    <div class="mb-3"><input type="text" class="form-control" id="addressDetail" placeholder="상세주소 (예: 101동 101호)"></div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="isDefaultCheck" checked>
                        <label class="form-check-label" for="isDefaultCheck">대표 배송지로 설정</label>
                    </div>
